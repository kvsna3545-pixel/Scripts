...
SettingsTab:Slider({
    ...
    Callback = function(value)
        jumpGlitchPower = value
    end
})

-- ระบบคำนวณการเคลื่อนที่
RunService.Heartbeat:Connect(function()
    if not jumpGlitchEnabled then return end
    ...
    if hum and root and hum.MoveDirection.Magnitude > 0 then
        -- ตรวจสอบว่าตัวละครกำลังกระโดดหรือลอยอยู่ (FloorMaterial เป็น Air)
        if hum.FloorMaterial == Enum.Material.Air then
            -- ผลักตัวละครไปข้างหน้าตามทิศทางที่กำลังเดิน
            root.Velocity = root.Velocity + (hum.MoveDirection * jumpGlitchPower * 0.2)
        end
    end
end)

-- ทำให้กระโดดทันทีที่แตะพื้น (ถ้ากด Space ค้างไว้)
UIS.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.Space then
        task.spawn(function()
            while UIS:IsKeyDown(Enum.KeyCode.Space) and jumpGlitchEnabled do
                local char = player.Character
                if char and char:FindFirstChild("Humanoid") then
